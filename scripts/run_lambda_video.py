...
for I in I_vals:
    ...
    animation = make_gif(
        r,
        plot_func=plot_trajectories_2d_ax,
        fps=video_fps,
        frame_stride=video_frame_stride,
        cumulative=True,
        trail_length=video_trail_length,
        title_func=lambda frame, I=I: (
            rf"$I_0={I:.2f},\quad "
            rf"t={frame * saved_dt:.1f}\,\mathrm{{s}}$"
        ),
        show=False,
        fig_kwargs={
            "figsize": (7.2, 4.2),
        ...
        excluded_axis=0,   # exclude x, so the plotted plane is y-z
        n_show=video_n_show,
        config=config,
    )
    ...
    # make_gif creates the current Matplotlib figure; close it after saving.
    plt.close()
