...
rm = ResultsManager(config_path=config_path, tag=f"cuboid_I={config["I"]}_collector")
...
# Plotting
pc = PlotCollector()
pc.add(plot_hist_ax, n_z, axis_label=fr"$\cos\theta$", bins=40, label=fr"Angular distribution of all swimmers, $I={config["I"]}$", bin_stats = True, show_count_fluct=True)
pc.add(plot_hist_ax, n_z_some, axis_label=fr"$\cos\theta$", bins=40, label=fr"Angular distribution of swimmers with $z>{frac} \cdot L_z$, $I={config["I"]}$")

rm.save_plot(pc.render(), name= "joint_fig")
